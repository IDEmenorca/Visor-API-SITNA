...
(function () {
    ...
    TC.inherit(TC.control.BirdEye, TC.Control);

    var ctlProto = TC.control.BirdEye.prototype;

    ctlProto.CLASS = 'tc-ctl-be';

    ctlProto.template = {};

    if (TC.isDebug) {
        ctlProto.template[ctlProto.CLASS] = TC.apiLocation + "TC/templates/BirdEye.html";
        ctlProto.template[ctlProto.CLASS + '-view'] = TC.apiLocation + "TC/templates/BirdEyeView.html";
    } else {
        ctlProto.template[ctlProto.CLASS] = function () { dust.register(ctlProto.CLASS, body_0); function body_0(chk, ctx) { return chk.w("<div class=\"tc-ctl-be-btn\" title=\"").h("i18n", ctx, {}, { "$key": "be.tip" }).w("\"><div class=\"tc-ctl-be-drag\"></div></div>"); } body_0.__dustBody = !0; return body_0 };
        ctlProto.template[ctlProto.CLASS + '-view'] = function () { dust.register(ctlProto.CLASS + '-view', body_0); function body_0(chk, ctx) { return chk.w("<div class=\"tc-ctl-be-btn-close\" title=\"").h("i18n", ctx, {}, { "$key": "closeBirdEye" }).w("\"></div>"); } body_0.__dustBody = !0; return body_0 };
    }
    ...
    var resolve = function (ctl) {
        ...
        if (dragRect.top < btnRect.top || dragRect.top > btnRect.bottom ||
            ...
            // Hemos soltado fuera del botón: activar BirdEye
            result = true;
    ...
    ctlProto.register = function (map) {
        ...
        if (!self._$viewDiv) {
            ...
            self._$viewDiv = $(self.viewDiv).attr('id', 'be-view');
        ...
        self._templatePromise.then(function () {
            ...
            $view.find('.' + self.CLASS + '-btn-close').on('mouseup', function (e) {
                ...
                /*Silme, amb la linia de baix eliminem un espai blanc de un px que quedava*/
                document.getElementById('be-view').style.position = "";
            });
        }
            , function (a, b, c) {
                TC.error("Error de renderizado BirdEye");
            });
    };
    ...
    ctlProto.render = function () {
        ...
        TC.Control.prototype.render.call(self, function () {
            if (dust.cache[self.CLASS + '-view']) {
                ...
                TC.error("No hay dust.cache para BirdEye");
            }
        });
    };
    ...
    ctlProto.callback = function (coords) {
        ...
        this._$viewDiv.removeClass(TC.Consts.classes.HIDDEN)
        this._$viewDiv.addClass(TC.Consts.classes.VISIBLE)
        $(this.map.div).addClass(TC.Consts.classes.COLLAPSED).trigger('resize');
    ...
    function GetMap2(coords) {

        var pos = new OpenLayers.LonLat(coords[0], coords[1]);
        var position = pos.transform(new OpenLayers.Projection("EPSG:3857"), new OpenLayers.Projection("EPSG:4326"));



        if (!mapBing) {
            var mapOptions = {
                credentials: "Al-jFWKatS_LKV7MzDxRubGwfTLsP9-HF8NRVasOZ_T3T6ipo4iZCnIvBVM_uxmZ", 
                center: new Microsoft.Maps.Location(position.lat, position.lon), 
                zoom: 20,
                showScalebar: true
            }

            mapBing = new Microsoft.Maps.Map(document.getElementById("be-view"), mapOptions);
        } else {
            var viewOptions = {
                center: new Microsoft.Maps.Location(position.lat, position.lon)
            }

            mapBing.setView(viewOptions);
        }

        Microsoft.Maps.Events.addHandler(mapBing, 'viewchangeend', function () { setMapCenter(); });
    }
})();

function setMapCenter()
{
    self.silmeMap.map.getView().setCenter(ol.proj.transform([mapBing.getCenter().longitude, mapBing.getCenter().latitude], 'EPSG:4326', 'EPSG:3857'));
    var zoom = mapBing.getZoom();
    zoom = zoom - 1;
    if (zoom >= 20)
        zoom = 19;

    self.silmeMap.map.getView().setZoom(zoom);
}