...
(function () {
    ...
    ctlProto.register = function (map) {
        ...
        map.on(TC.Consts.event.BEFORELAYERADD + ' ' + TC.Consts.event.BEFOREUPDATEPARAMS, function (e) {
            ...
            if (!layer.isBase && layer.type === TC.Consts.layerType.WMS) {
                self._readyDeferred.then(function () { // Esperamos a que cargue primero las capas de la configuración
                    ...
                    var updateControl = function () {
                        ...
                        for (var i = 0, len = $(self.getLayerNodes(layer)).length; i < len; i++) {
                            if ($((self.getLayerNodes(layer))[i]).hasClass(TC.Consts.classes.LOADING)) {
                                $((self.getLayerNodes(layer))[i]).removeClass(TC.Consts.classes.LOADING).addClass(TC.Consts.classes.CHECKED).find('span').attr(TOOLTIP_DATA_ATTR, layerAddedText);
                            }
                        }
                        _refreshResultList();
                    };
                    if ($layerNode.length) {
                        updateControl();
                        //Silme
                        $("[data-tc-layer-uid='" + $layerNode[0].attributes[2].value + "']").removeClass(TC.Consts.classes.LOADING)
                        $("[data-tc-layer-uid='" + $layerNode[0].attributes[2].value + "']").addClass(TC.Consts.classes.CHECKED)
                        $("[data-tc-layer-uid='" + $layerNode[0].attributes[2].value + "']").addClass(TC.Consts.classes.CHECKED).find('span').attr(TOOLTIP_DATA_ATTR, layerAddedText);
            ...
            //Silme:
            var $layerNode = $(self.getLayerRootNode(e.layer));
            if ($layerNode[0] != null) {
                $("[data-tc-layer-uid='" + $layerNode[0].attributes[2].value + "']").removeClass(TC.Consts.classes.CHECKED)
                $("[data-tc-layer-uid='" + $layerNode[0].attributes[2].value + "']").find('span').attr(TOOLTIP_DATA_ATTR, clickToAddText);
        ...
        self._$div.on(TC.Consts.event.CLICK, 'span', function (e) {
            ...
            if (!$li.hasClass(TC.Consts.classes.LOADING) && !$li.hasClass(TC.Consts.classes.CHECKED)) {
                ...
                //Silme
                var layerUid = $li[0].attributes[2].value;
                if (!layer) {
                ...
                if (layer && layerName) {
                    ...
                    $('#bma').addClass(TC.Consts.classes.COLLAPSED);//Silme
                    $('#xdata').addClass(TC.Consts.classes.COLLAPSED);//Silme
                    $('#toc').removeClass(TC.Consts.classes.COLLAPSED);//Silme
                }
            }
        });

        self._$div.on(TC.Consts.event.CLICK, 'li', function (e) { //Silme
            var $li = $(e.target);

            //            if (!$li.hasClass(TC.Consts.classes.LOADING) && !$li.hasClass(TC.Consts.classes.CHECKED) && $(e.target).hasClass('tc-ctl-lcat-leaf')) {//Silme
            if (!$li.hasClass(TC.Consts.classes.LOADING) && !$li.hasClass(TC.Consts.classes.CHECKED) && $(e.target).hasClass(TC.Consts.classes.SELECTABLE)) {//Silme
                e.preventDefault;
                ...
                if (!layer) {
                    for (var k = 0; k < self.layers.length; k++) {
                        for (var m = 0; m < self.layers[k].tree.children.length; m++) {
                            if (self.layers[k].tree.children[m].uid == $li[0].attributes[2].value) {
                                layer = self.layers[k];
                                break;
                            }
                            if (self.layers[k].tree.children[m].children.length > 0) {
                                for (var p = 0; p < self.layers[k].tree.children[m].children.length ; p++) {
                                    if (self.layers[k].tree.children[m].children[p].uid == $li[0].attributes[2].value) {
                                        layer = self.layers[k];
                                        break;
                                    }
                                }
                ...
                //REVISAR SI ES NECESSARI AQUI O BASTA NOMES AMB EL DE DALT
                //var layerUid = $li[0].attributes[2].value;
                var layer = "";
                var layerUid = $li.parent()[0].attributes[0].value;
                if (!layer) {
                    for (var k = 0; k < treeLayers.length; k++) {
                        var rama = treeLayers[k].tree;
                        cercaNode(rama, layerUid);
    ...
    ctlProto.render = function (callback) {
        ...
        $.when.apply(self, deferreds).then(function () {
            ...
            var layerTrees = $.map(self.layers, function (layer) {
                ...
                //=====================================
                //SILME: Añadimos parentNode al layerTree
                //=====================================
                try {
                    result.parentNode = layer.parentNode;
                }
                catch (err) {
                    console.log("no hay parentNode");
                }
                ...
                if (!self.searchInit) {
                    ...
                    self._$div.on("click", "h2 button", function () {
                        ...
                        if (searchPane.hasClass(TC.Consts.classes.HIDDEN) || wasCollapsed) {
                            ...
                            $('.tc-ctl-lcat-search li button.tc-checked').removeClass(TC.Consts.classes.CHECKED);
                            //}
                        }
                        else {
                            ...
                            /*Silme var selectedCount = $('.tc-ctl-lcat-tree li button.tc-checked').length;
                            if (selectedCount > 0) {
    ...
    ctlProto.getLayerRootNode = function (layer) {
        ...
        if (!layer.isBase) {
            ...
            if (self._$roots) {
                ...
                if (layer.names != null) {
                    for (var i = 0; i < layer.names.length; i++) {
                        $('.tc-ctl-lcat-tree li').each(function (indice, elemento) {
                            if (elemento.attributes.length > 2) {
                                if (elemento.attributes[1].value == layer.names[i]) {
                                    result = elemento;
                                }
                            }
                        });
                ...
                if (result==null){
                    for (var k=0; k < self.layers.length; k++){
                        for (var m=0; m < self.layers[k].tree.children.length; m++){
                            if (self.layers[k].tree.children[m].name == layer.names[0]){
                                selectedNode = self.layers[k].tree.children[m].uid;
                                break;
                            }
                        }
                    } 
                    if (selectedNode==""){
                        for (var k=0; k < self.layers.length; k++){
                            for (var m=0; m < self.layers[k].tree.children.length; m++){
                                if (self.layers[k].tree.children[m].children.length>0){
                                    for (var p=0; p < self.layers[k].tree.children[m].children.length ;p++){
                                        if (self.layers[k].tree.children[m].children[p].name == layer.names[0]){
                                            selectedNode = self.layers[k].tree.children[m].children[p].uid;
                                            break;
                                        }
                                    }
                    ...
                    if (selectedNode!=""){
                        $('.tc-ctl-lcat-tree li').each(function(indice, elemento) {
                            if (elemento.attributes.length>2){
                                if (elemento.attributes[2].value == selectedNode){
                                    //nodeId = elemento.attributes[2].value;
                                    result = elemento;
                                }
                            }
                        });                        
    ...
    ctlProto.showLayerInfo = function (layer, name) {
        ...
        */
        ...
        //HERE
        //layerName = $(this).parent()[0].attributes[1].value;
        layerName = name;

        /*
        //hem de trobar l'id de la capa en questió (name)
        //NOT GOOD
        for (var i = 0; i < $(this).parent().prevObject[0].layers.length; i++)
        {
            for (var j = 0; j < $(this).parent().prevObject[0].layers[i].)
            $(this).parent().prevObject[0].layers[i];
        }
        //END NOT GOOD
        */
        //GOOD
        ...
        {
            for (var j in this.layers[i]._capabilitiesNodes)
            {
                if (this.layers[i]._capabilitiesNodes[j].Name == name) //TROBAT
                {
                    layerUid = j;
                }
            }
        ...
        //END GOOD
        //this.layers[0].compatibleLayers
        
            

        //layerUid = $(this).parent()[0].attributes[2].value;//DELETE
