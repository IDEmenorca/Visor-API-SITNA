...
(function () {
    ...
    ctlProto.updateLayerVisibility = function (layer) {
        ...
        if ($li) {
            ...
            //Silme $li.find('input[type="checkbox"]').prop('checked', visible);
            //Silme $li.find('.' + self.CLASS + '-del').toggleClass(TC.Consts.classes.HIDDEN, visible);
        ...
        if (!layer.isBase && !layer.options.stealth) {
            ...
            if (!alreadyExists) {
                            ...
                            function () {
                                ...
                                if (isRaster) {
                                    ...
                                    if (layer.layerNames[0].includes(":"))
                                        layerData.layerid = layer.layerNames[0].substr(layer.layerNames[0].indexOf(":") + 1);
                                    else
                                        layerData.layerid = layer.layerNames[0];
                                    //End Silme
                                    //layerData.layerid = layer.layerNames[0];//Silme
                                    var hasInfo = (info.hasOwnProperty('abstract') || info.hasOwnProperty('legend') || info.hasOwnProperty('metadata') || dataUrl);
                                    //SILME
                                    var metadata;
                ...
                if (elligibleLayersNum == 0)
                    $('.' + self.CLASS + '-n').text(elligibleLayersNum).addClass('silme-invisible');
                else
                    $('.' + self.CLASS + '-n').text(elligibleLayersNum).removeClass('silme-invisible');
                //End Silme
    ...
    ctlProto.removeLayer = function (layer) {
        ...
        if (nChildren == 0)
            $('.' + self.CLASS + '-n').text(nChildren).addClass('silme-invisible');
        else
            $('.' + self.CLASS + '-n').text(nChildren).removeClass('silme-invisible');
    };
